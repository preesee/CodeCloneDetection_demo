...
from keras.layers import Input, Embedding, LSTM, Lambda, Dense,Activation
...
n_cfg_hidden=64
normalized=False
print("params: hidden: %s, batch_size: %d , epoche : %d, nomorlized: %s, n_cfg_hidden : %d" % (n_hidden,batch_size,n_epoch,str(normalized),n_cfg_hidden))
...
print("left_input_txt.shape: "+str(left_input.shape))
cfg_left_input = Input(shape=(1,),dtype='int32')

print("cfg_left_input.shape: "+str(cfg_left_input.shape))
...
print("cfg_embedding_l.shape: "+str(cfg_embedding_l.shape))
...
print("encoded_left_txt.shape: "+str(encoded_left.shape))
...
left_output=krs.layers.Concatenate(axis=1)([encoded_left,cfg_embedding_l])
print("left_output.shape: "+str(left_output.shape))
right_output=krs.layers.Concatenate(axis=1)([encoded_right,cfg_embedding_r])
shared_lstm = LSTM(n_hidden,return_sequences=False)

#shared_cfg_lstm = LSTM(n_cfg_hidden,return_sequences=False)

bn=BatchNormalization()
#encoded_left=krs.layers.Reshape((2,))(encoded_left)
#encoded_right=krs.layers.Reshape((2,))(encoded_right)
left_output = shared_lstm(left_output)

#left_output=bn(left_output) # normalization

right_output = shared_lstm(right_output)
#right_output=bn(right_output)
#state_cfg_h_l=shared_cfg_lstm(cfg_embedding_l)
#state_cfg_h_r=shared_cfg_lstm(cfg_embedding_r)
#print("state_h_l.shape: "+str(state_h_l.shape))
#print("state_cfg_h_l.shape: "+str(state_cfg_h_l.shape))
...
if normalized:
    classify = Dense(5, kernel_initializer='uniform')(malstm_distance)
    classify = bn(classify)
    act= Activation('softmax')
    classify=act(classify)

else:
    classify = Dense(5, kernel_initializer='uniform')(dist)
    classify = bn(classify)
    act= Activation('softmax')
    classify = act(classify)
...
optimizer=krs.optimizers.Adam(lr=0.001)
...
modelname=MODEL_SAVING_DIR+str(datetime.datetime.now())+'CFG_txt_concat_before_LSTM+txt.model.pkl'
