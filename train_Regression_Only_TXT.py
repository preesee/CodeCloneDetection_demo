...
TRAIN_PAIRS_JSON_FILE = "C:\\work\\codeclone_data\\TRAIN_txt_PAIRS_JSON_FILE.json"
# TEST_CSV = '..../test.csv'
...
def create_CFG_structor(clone_data, dotfile_data_dict):
    ...
    source_plain_text = read_source_code(data[2], data_tuple[1] - 1, data_tuple[2])
...
if rebuild_json_cfg:
    with open(clone_pairs_file, mode='r') as csv_file:  # data pair source code
        ...
        for item in csv_reader:
            ...
            try:
                data_dic = {}
                source_plain_text_a = create_CFG_structor(item['CLONE_NAME1'], dotfile_data_dict)
                source_plain_text_b = create_CFG_structor(item['CLONE_NAME2'], dotfile_data_dict)

                data_dic['code_clone1'] = source_plain_text_a
                data_dic['code_clone2'] = source_plain_text_b

                data_dic['TYPE'] = item['TYPE']
                data_dic['sim_score'] = item['SIMILARITY_LINE']
...
n_epoch = 50
normalized = False
print("params: hidden: %s, batch_size: %d , epoche : %d, nomorlized: %s" % (
    n_hidden, batch_size, n_epoch, str(normalized)))
...
#
# embedding_cfg_layer = Embedding(len(cfg_embedding_matrix), embedding_dim, weights=[cfg_embedding_matrix],
#                                 input_length=1,
#                                 trainable=False)
...
# classify = Dense(5, activation=softMaxAxis1)(malstm_distance)


# if normalized:
#     classify = Dense(5, activation='softmax')(malstm_distance)
# else:
...
#malstm.compile(loss='mean_squared_error', optimizer=optimizer,metrics=[categorical_accuracy])  # , f1, recall, precision])
...
scores = malstm.evaluate([X_test['left'], X_test['right']], Y_test)
...
#
# plt.savefig(FIG_SAVING_DIR+str(datetime.datetime.now())+'acc_TXT.png')
# #plt.show()
#
# # Plot loss
...
# plt.savefig(FIG_SAVING_DIR+str(datetime.datetime.now())+'loss_TXT.png')
# plt.show()
...
def plot_history(history):
    ...
    plt.ylabel('Loss')
    plt.legend()
    plt.savefig(FIG_SAVING_DIR + str(datetime.datetime.now()) + 'loss_txt_only.png')
    ## Accuracy
    plt.figure(2)
    for l in acc_list:
        plt.plot(epochs, history.history[l], 'b',
    ...
    plt.ylabel('Accuracy')
    plt.legend()
    plt.savefig(FIG_SAVING_DIR + str(datetime.datetime.now()) + 'acc_txt_only.png')
    plt.show()
...
# with open(FIG_SAVING_DIR+str(datetime.datetime.now())+'loss_txt_only.model.json',"w") as json_file:
#     json_file.write(malstm_trained_json)
# malstm_trained.model.save_weights(FIG_SAVING_DIR+str(datetime.datetime.now())+'loss_txt_only.model.h5')
# print("save model to disk done")

hist = malstm_trained.history
modelname = MODEL_SAVING_DIR + str(datetime.datetime.now()) + 'txt_only.model.pkl'

with open(modelname, "wb") as file_pi:
    pickle.dump(hist, file_pi)
print("txt only model save done!")
