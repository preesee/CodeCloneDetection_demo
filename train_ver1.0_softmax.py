...
malstm = Model([left_input, right_input], [classify])
...
metrics = Metrics()
malstm_trained = malstm.fit([X_train['left'], X_train['right']], keras.utils.np_utils.to_categorical(Y_train,5), batch_size=batch_size, nb_epoch=n_epoch,
                           # callbacks=[metrics],
                           validation_data=([X_validation['left'], X_validation['right']], keras.utils.np_utils.to_categorical(Y_validation,5)))

print("Training time finished.\n{} epochs in {}".format(n_epoch,
                                                        datetime.timedelta(seconds=time() - training_start_time)))
scores = malstm.evaluate([X_validation['left'], X_validation['right']], keras.utils.np_utils.to_categorical(Y_validation,5))
