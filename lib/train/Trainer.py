...
class Trainer(object):
    ...
    def train_epoch(self, epoch):
        ...
        # print "self.train_data.numBatches: ", self.train_data.numBatches
        batch_order = torch.randperm(len(self.train_data))
        for i in range(len(self.train_data)): #
            batch = self.train_data[i] # batch_order[i]
            # print "src_batch-data: "
            # print batch[0][0].data.cpu()
            # print "leaf_batch-data: "
            # print batch[1][0].data
            # print batch[1][0].data.cpu()[:, 0]
            ...
            if self.opt.has_attn:
                if self.opt.data_type == 'code' or self.opt.data_type == 'text':
                   self.model.decoder.attn.applyMask(attention_mask)
                    ...
                    # self.model.decoder.code_attn.applyMask(attention_mask_code)
                    # self.model.decoder.text_attn.applyMask(attention_mask_txt)
