...
datafortrain += datarowsT1[:4000]
datafortrain += datarowsT2[:4000]
...
X_validation = {'left': X_validation.code_clone1, 'right': X_validation.code_clone2,
                # 'cfg_A': [doc2vec_cfg[cfg] for cfg in cfg_idx_A_validation],
                # 'cfg_B': [doc2vec_cfg[cfg] for cfg in cfg_idx_B_validation]}
...
cfg_left_input = Input(shape=(1,),dtype='int16')
#cfg_right_input = Input(shape=(max_seq_length,))
cfg_right_input = Input(shape=(1,),dtype='int16')
...
malstm.compile(loss='categorical_crossentropy', optimizer='adam', metrics=['accuracy', f1, recall, precision])
...
malstm_trained = malstm.fit(
    ...
    # callbacks=[metrics],
    validation_data=(
        [X_validation['left'], X_validation['right'], X_validation['cfg_A'],
         X_validation['cfg_B']], krs.utils.to_categorical(Y_validation, 5)))

print("Training time finished.\n{} epochs in {}".format(n_epoch,
                                                        datetime.timedelta(seconds=time() - training_start_time)))
scores = malstm.evaluate([X_validation['left'], X_validation['right'], X_validation['cfg_A'],
                          X_validation['cfg_B']], krs.utils.to_categorical(Y_validation, 5))
