...
label_data_convertor['T4'] = 5
label_data_convertor['T2'] = 2
label_data_convertor['T1'] = 1
label_data_convertor['MT3'] = 3
label_data_convertor['ST3'] = 4
label_data_convertor['NG'] = 0
...
datarow_neg = []
for i in range(len(data_loaded_cp)-10):
    neg_sample_data={}
    neg_sample_data=data_loaded_cp[i]
    neg_sample_data['cfg_idx_B']=data_loaded_cp[i+5]['cfg_idx_B']
    neg_sample_data['code_clone2']=data_loaded_cp[i+5]['code_clone2']
    neg_sample_data['TYPE']='NG'
    datarow_neg.append(neg_sample_data)

datafortrain = []
datafortrain += datarowsT4[:8000]
datafortrain += datarowsT1[:]
datafortrain += datarowsT2[:]
datafortrain += datarowsMT3[:8000]
datafortrain += datarowsST3[:8000]
datafortrain += datarow_neg[:8000]
# Load training and test set

# test_df = ''  # pd.re ad_csv(TEST_CSV)
...
encoded_left = encoded_left#embedding_layer(left_input)
encoded_right = encoded_right#embedding_layer(right_input)
...
classify = Dense(classification_num, activation=softMaxAxis1)(dist)
...
scores = malstm.evaluate([X_test['left'], X_test['right'], X_test['cfg_A'],
                          X_test['cfg_B']], krs.utils.to_categorical(Y_test, classification_num))
