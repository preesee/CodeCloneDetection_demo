...
state_h_l=bn(state_h_l) # normalization

state_h_r = shared_lstm(encoded_right)
state_h_r=bn(state_h_r)
...
left_output=krs.layers.Concatenate()([state_h_l,state_cfg_h_l])
print("left_output.shape: "+str(left_output.shape))
right_output=krs.layers.Concatenate()([state_h_r,state_cfg_h_r])
...
# modelname=FIG_SAVING_DIR+str(datetime.datetime.now())+'losscfg+txt.model.json'
# with open(modelname,"w") as json_file:
#     json_file.write(malstm_trained_json)
# modelwightname=FIG_SAVING_DIR+str(datetime.datetime.now())+'loss_cfg+txt_only.model.h5'
...
modelname=MODEL_SAVING_DIR+str(datetime.datetime.now())+'cfg+txt_concat_after_LSTM.model.pkl'
